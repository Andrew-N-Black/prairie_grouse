...
#
# There is no ROHan biocontainers module on Gautschi, and it's not on
# bioconda either, so it needs to be compiled. Run this ON THE LOGIN NODE
# (plain `bash install_rohan.sh`, NOT sbatch) — compute nodes may not have
# outbound internet access for the `git clone`, and this build takes under
# a minute regardless, so there's no need to queue it.
#
# ROHan works directly from a BAM/CRAM + reference FASTA -- it computes its
# own genotype likelihoods internally via a Bayesian model + HMM, rather
# than taking pre-called genotypes. It does NOT accept a VCF as input.
# =============================================================================
set -euo pipefail
...
# ROHan needs a C++ compiler. gcc/14.1.0 is confirmed present and is what
# built htslib/libharu successfully in earlier attempts.
module --force purge
