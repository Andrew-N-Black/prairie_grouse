...
# Step 08 — requires 07_downsample_and_finalize.sh to have completed
...
#   sbatch --array=0-$((N-1))%20 08_heterozygosity_array.sh
# =============================================================================
...
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    ...
    echo "Submit with: sbatch --array=0-N 08_heterozygosity_array.sh"
    exit 1
fi

if [[ ! -f "$FINAL_BAMLIST" ]]; then
    echo "ERROR: ${FINAL_BAMLIST} not found."
    echo "Run 07_downsample_and_finalize.sh first."
    exit 1
fi
...
# tasks to race on. 09_pca_roh.sh (or `cat *_heterozygosity.txt` directly)
