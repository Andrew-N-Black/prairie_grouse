...
# Step 06 of the replication/extension of Black et al. (PNAS Nexus)
...
#     shortread_manifest.tsv (built for 05_shortread_introgression.sh).
#
...
#        sbatch --array=0-0 06_combined_alignment_array.sh --build-list-only
#
#   2. Submit the full array (the prep step above prints the exact N):
#        N=$(grep -vc '^#' <PROJECT_DIR>/combined_samples.tsv)
#        sbatch --array=0-$((N-1))%20 06_combined_alignment_array.sh
# =============================================================================
...
build_combined_samples() {
    ...
    echo "    sbatch --array=0-$((total - 1))%20 06_combined_alignment_array.sh"
}
...
if [[ ! -f "$COMBINED_SAMPLES" ]]; then
    ...
    echo "Run: sbatch --array=0-0 06_combined_alignment_array.sh --build-list-only"
    exit 1
fi
...
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    ...
    echo "Submit with: sbatch --array=0-N 06_combined_alignment_array.sh"
    exit 1
fi
