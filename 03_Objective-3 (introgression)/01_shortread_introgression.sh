...
# Step 05 — requires 04_pangenome_analysis.sh to have completed (consumes its
...
#
# USAGE:
#   sbatch 05_shortread_introgression.sh
# =============================================================================
...
PANGENOME_DIR="${PROJECT_DIR}/pangenome"   # input: from 04_pangenome_analysis.sh
...
echo ">>> 05_shortread_introgression.sh"
echo ">>> Node      : $(hostname)"
echo ">>> CPUs      : ${THREADS}"
...
if [[ -z "$GFA" ]]; then
    ...
    echo "Run 04_pangenome_analysis.sh first."
    exit 1
fi
...
if [[ "$N_REF_CONTIGS" -eq 0 ]]; then
    ...
    echo "04_pangenome_analysis.sh built the combined FASTA."
    exit 1
fi
