...
# Step 02 — requires 01_download_chicken_ref.sh to have completed first.
...
#
# Per sample:
#   1. Raw HiFi reads are adapter-filtered/trimmed with HiFiAdapterFilt —
#      even a small fraction of adapter-contaminated reads can cause
#      chimeric joins in hifiasm, so this runs before assembly, not after.
#   2. hifiasm assembles the filtered HiFi reads, phased into two haplotypes
#      using the sample's Hi-C reads (--h1/--h2 Hi-C integration mode). The
#      3 samples that also have Oxford Nanopore ultra-long reads pass them
#      in via --ul for extra contiguity.
#   3. Each haplotype's contigs are Hi-C scaffolded with yahs: Hi-C reads are
...
#   4. A .hic file is built from yahs's raw scaffolding output (before RagTag
#      touches it) for interactive inspection/manual curation in Juicebox.
#   5. RagTag orders and orients the yahs scaffolds into pseudo-chromosomes
#      against the chicken (Gallus gallus, GRCg7b) reference.
#   6. Liftoff transfers chicken gene annotations onto each pseudo-chromosome
#      assembly.
#   7. QC: a minimap2-vs-chicken dotplot validates scaffold order/orientation
...
#     - hifi_bams   : one or more raw PacBio HiFi BAM paths (the
#                     *.hifi_reads.bc####.bam files straight off the
#                     instrument — NOT aligned to anything; bc#### is a
#                     demultiplexing barcode, not an alignment target),
#                     comma-separated if multiple SMRT cells. HiFiAdapterFilt
#                     (Step 1) converts these to filtered FASTQ itself.
...
# 01_download_chicken_ref.sh (GCF_016699485.2 / GRCg7b); used here by RagTag
...
# of yahs's raw scaffolding (Step 6, below). NOT an HPC module: download once
...
# HiFiAdapterFilt — also no module; also installed via conda/bioconda (pulls
# in blast/bamtools/pigz automatically as package dependencies). Unlike the
# tools above, this one is ACTIVATED rather than invoked by absolute path:
# its bioconda packaging relies on conda's activate hook to put its bundled
# adapter database directory on PATH, so an absolute-path invocation alone
# would not find it. Activation is scoped tightly around Step 1 only (see
# there) to avoid reintroducing the kind of PATH-shadowing risk noted above.
...
mkdir -p logs "$ASM_DIR" "$SCAFFOLD_DIR" "$RAGTAG_DIR" "$LIFTOFF_DIR" "$FINAL_DIR" "$REF_DIR" "$QC_DIR" "$FILT_DIR"

# =============================================================================
# INSTALL yahs + Pretext + dotplot Python env + HiFiAdapterFilt (once —
# skipped on reruns if already present). `anaconda` is loaded only for
# these calls and unloaded immediately after — see the NOTE in ENVIRONMENT
# SETUP above for why it can't stay loaded alongside liftoff (and possibly
# other) modules used later in this script.
# =============================================================================
NEED_ANACONDA=false
...
# Downloaded by 01_download_chicken_ref.sh — must be run before this script.
# =============================================================================
if [[ ! -f "$CHICKEN_FASTA" || ! -f "$CHICKEN_GFF" ]]; then
    echo "ERROR: Chicken reference not found at ${REF_DIR}"
    echo "Run 01_download_chicken_ref.sh first, then submit this array."
    exit 1
fi
...
if [[ -z "$SAMPLE" || -z "$HIFI_BAMS" || -z "$HIC_R1" || -z "$HIC_R2" ]]; then
